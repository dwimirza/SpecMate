// src/scripts/scrapeGpu.ts
import * as cheerio from 'cheerio'
import { prisma } from '@/lib/prisma'
import { detectBrand, slugify } from '@/lib/slugify'

async function main() {
  const res = await fetch('https://www.videocardbenchmark.net/gpu_list.php', {
    headers: { 'user-agent': 'Mozilla/5.0' }
  })

  if (!res.ok) {
    throw new Error(`Failed GPU page: ${res.status}`)
  }
  ...
  console.log(`Upserted ${rows.length} GPUs`)
}
